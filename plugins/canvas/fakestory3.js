...
async function roundedRectPath(ctx, x, y, width, height, radius) {
  ctx.beginPath();
...
async function createFakeStory(
  ...
  const { createCanvas, loadImage, Path2D } = await _getCanvas();
  const height = 1150;
  ...
  async function drawSvgOutline(pathData, x, y, scale) {
    ctx.save();
    ctx.translate(x, y);
