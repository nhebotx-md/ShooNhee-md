...
import fs from 'fs'
import { delay, DisconnectReason, jidNormalizedUser, useMultiFileAuthState } from 'ShooNhee'
...
async function startJadibot(sock, m, userJid, usePairing = true) {
    ...
    const { default: makeWASocket, fetchLatestBaileysVersion, makeCacheableSignalKeyStore } = await import('ShooNhee')
    ...
    if (usePairing && !state.creds?.registered) {
        try {
            ...
            if (m && m.chat) {
                let thumbnail = null
                try {
                    if (fs.existsSync('./assets/images/ShooNhee2.jpg')) {
                        thumbnail = fs.readFileSync('./assets/images/ShooNhee2.jpg')
                    }
                } catch {}
    ...
    childSock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            if (msg.key && msg.key.remoteJid === '13135550002@s.whatsapp.net') continue
