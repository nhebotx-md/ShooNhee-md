...
async function startSewaChecker(sock) {
    ...
    if (activeIntervals.has('sewaChecker')) {
        clearInterval(activeIntervals.get('sewaChecker'));
        activeIntervals.delete('sewaChecker');
    }
    ...
    activeIntervals.set('sewaChecker', intervalId);
    logger.info('Scheduler', 'Sewa checker enabled (every 10 minutes)');
}

export { initScheduler, stopAllSchedulers, startDailyLimitReset, startGroupScheduleChecker, startSewaChecker, scheduleMessage, cancelScheduledMessage, getScheduledMessages, getScheduledMessage, saveScheduledMessages, loadScheduledMessages, getMsUntilTime, formatTimeRemaining, getSchedulerStatus, getFullSchedulerStatus, isSchedulerRunning, startSchedulerByName, stopSchedulerByName }