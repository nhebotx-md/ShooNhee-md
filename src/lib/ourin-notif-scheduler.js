...
function getCurrentWIB() {
    ...
    return { time: `${h}:${m}`, date: `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}` }
}
...
function initNotifScheduler(socketInstance) {
    ...
    logger.info('NotifScheduler', 'Meal & sleep notification scheduler started')
}
...
async function checkNotifications() {
    ...
    for (const [key, entry] of Object.entries(makanData)) {
        ...
        await processEntry(entry, time, date, 'makan', key, db)
    }

    for (const [key, entry] of Object.entries(tidurData)) {
        if (!entry.enabled) continue
        await processEntry(entry, time, date, 'tidur', key, db)
    }
}

async function processEntry(entry, currentTime, currentDate, type, key, db) {
    if (!entry.jadwal || !Array.isArray(entry.jadwal)) return

    const lastSent = entry.lastSent || {}

    for (const jam of entry.jadwal) {
        if (currentTime !== jam) continue

        const sentKey = `${jam}_${currentDate}`
