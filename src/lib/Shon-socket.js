...
import { addExifToWebp, DEFAULT_METADATA } from "./Shon-exif.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "ShooNhee-AI";
    ...
    for (let i = 0; i < stickers.length; i++) {
      try {
        ...
        stickerFiles[fileName] = webpBuffer;
        stickerMeta.push({
          fileName,
          mimetype: "image/webp",
          isAnimated: isGif,
    ...
    for (const [name, buf] of Object.entries(stickerFiles)) {
      fs.writeFileSync(path.join(packDir, name), buf);
    }

    const zipPath = path.join(tempDir, `${stickerPackId}.zip`);
    ...
    const hkdfInfo = "WhatsApp Sticker Pack Keys";
    const importedKey = await globalThis.crypto.subtle.importKey(
      "raw", mediaKey, { name: "HKDF" }, false, ["deriveBits"],
    );
    const derivedBits = await globalThis.crypto.subtle.deriveBits(
      { name: "HKDF", hash: "SHA-256", salt: new Uint8Array(0), info: new TextEncoder().encode(hkdfInfo) },
      importedKey, 112 * 8,
    );
    const expandedKey = Buffer.from(derivedBits);
    ...
    sha256Plain.update(zipBuffer);
    const fileSha256 = sha256Plain.digest();

    const encrypted = Buffer.concat([aes.update(zipBuffer), aes.final()]);
    ...
    const encFilePath = path.join(tempDir, `sticker-pack_${Date.now()}-enc`);
    fs.writeFileSync(encFilePath, Buffer.concat([encrypted, mac]));

    const uploadResult = await sock.waUploadToServer(
      encFilePath,
      {
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "ShooNhee-AI";
    if (source) {
      let data = source;
  ...
  sock.getName = async (jid, groupJid = null) => {
    ...
    if (id === "13135550002@s.whatsapp.net") return "WhatsApp";
