...
/**
 * Safe wrapper untuk mengeksekusi fungsi tanpa melempar error.
 * @param {Function} fn - Fungsi yang akan dieksekusi.
 * @returns {any|null} Hasil fungsi atau null jika error.
 */
const safe = (fn) => {
  try {
    return fn();
...
// =============================================================================
// SECTION 2: DYNAMIC PLUGIN IMPORTS (Lazy Loading)
// =============================================================================
...
// =============================================================================
// SECTION 3: GLOBAL STATE & CACHE
// =============================================================================

/** Tracker untuk spam delay per user (key: `${chat}_${sender}`). */
const spamDelayTracker = new Map();

// Cleanup spam delay tracker setiap 30 detik.
setInterval(() => {
  const now = Date.now();
  for (const [key, timestamp] of spamDelayTracker) {
    if (now - timestamp > 15000) spamDelayTracker.delete(key);
  }
}, 30000);

/** Cache untuk gambar smart trigger. */
let _smartTriggerThumb = undefined;

/**
 * Mengambil buffer gambar untuk smart trigger (dengan caching).
 * @returns {Buffer|null}
 */
function getSmartTriggerThumb() {
  ...
  try {
    const imgPath = "./assets/images/ShooNhee2.jpg";
    _smartTriggerThumb = fs.existsSync(imgPath) ? fs.readFileSync(imgPath) : null;
  } catch {
...
/** Global rate limiter untuk anti-spam. */
...
/** Cache untuk plugin game yang aktif. */
const cachedGamePlugins = new Map();

// Pre-load semua plugin game saat startup.
try {
...
// =============================================================================
// SECTION 4: GAME ANSWER HANDLER
// =============================================================================

/**
 * Menangani jawaban dari session game yang sedang aktif.
 * @param {Object} m - Pesan yang sudah di-serialize.
 * @param {Object} sock - Socket connection.
 * @returns {Promise<boolean>} True jika game handler memproses pesan.
 */
async function handleGameAnswer(m, sock) {
  try {
    // Prioritaskan sulap plugin jika ada session-nya.
    if (sulapPlugin?.answerHandler) {
      const handled = await sulapPlugin.answerHandler(m, sock);
      if (handled) return true;
    }

    // Cek apakah ada session game aktif di chat ini.
    if (!hasActiveSession(m.chat)) return false;

    const session = getSession(m.chat);
    if (!session) return false;

    // Delegasikan ke plugin game yang sesuai.
...
// =============================================================================
// SECTION 5: SMART TRIGGER HANDLER
// =============================================================================

/**
 * Menangani auto-reply dan smart trigger berdasarkan keyword.
 * @param {Object} m - Pesan yang sudah di-serialize.
 * @param {Object} sock - Socket connection.
 * @param {Object} db - Database instance.
 * @returns {Promise<boolean>} True jika ada trigger yang cocok.
 */
async function handleSmartTriggers(m, sock, db) {
  ...
  // Abaikan jika pesan adalah command konfigurasi smart trigger.
  if (
  ...
  // Handler khusus untuk keyword "done" (transaction done).
  if (text === "done") {
  ...
  // Handler untuk session registrasi.
  ...
  try {
    ...
    /**
     * Memproses custom reply dari setting group atau global.
     * @param {Object} replyItem - Item custom reply.
    ...
    // --- Default Smart Trigger Responses ---
    if (isMentioned) {
      await m.reply(
    ...
    if (text?.toLowerCase() === "bot") {
      await m.reply(
        `Hai @${m.sender.split("@")[0]}, ${botName} Aktif ✅`,
        { mentions: [m.sender] },
      );
      return true;
    }
    ...
    if (text?.toLowerCase()?.includes("assalamualaikum")) {
      await m.reply(
        `Waaalaikumssalam @${m.sender.split("@")[0]}`,
        { mentions: [m.sender] },
      );
      return true;
    }
    ...
    if (text?.toLowerCase()?.includes("hallo")) {
      await m.reply(
        `Halo juga kak @${m.sender.split("@")[0]}`,
        { mentions: [m.sender] },
      );
      return true;
    }
...
// =============================================================================
// SECTION 6: ANTI-SPAM CHECKER
// =============================================================================

/**
 * Cek apakah user sedang spam.
 * @param {string} jid - JID user.
 * @returns {Promise<boolean>} True jika sedang spam.
 */
...
// =============================================================================
// SECTION 7: MAIN MESSAGE HANDLER
// =============================================================================
 ...
 * @param {boolean} [options.isJadibot=false] - Flag jadibot.
 * @param {string|null} [options.jadibotId=null] - ID jadibot.
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    // --- Debug Logging (Hardcoded Number) ---
    ...
    // --- Sticker Command Handler (Non-text Commands) ---
    if (!m.isCommand) {
      ...
      if (!m.isCommand) {
        // Fallback game answer tanpa quoted message.
    ...
    // --- Case Handler (Legacy Command System) ---
    try {
    ...
    // Fallback ke store command (private atau group non-store mode).
    if (!plugin) {
      if (storeCommand) {
        storeData[m.command.toLowerCase()].views =
          (storeCommand.views || 0) + 1;
          ...
          await sock.sendMessage(
            m.chat,
            {
              interactiveMessage: {
                ...
                document: getCachedThumb("./assets/images/ShooNhee.jpg"),
    ...
    // --- Group Mode & Jadibot Restriction ---
    if (m.isGroup) {
      const groupData = db.getGroup(m.chat) || {};
      ...
      // Jadibot: override mode ke md + block beberapa kategori & command.
      if (isJadibot) {
        botMode = "md";
    ...
    if (energiEnabled && plugin.config.energi > 0) {
      ...
      if (
        ...
        // Owner bypass energi check.
      } else if (m.isPremium && (premiumEnergi === -1 || user?.energi === -1)) {
        // Premium bypass energi check.
      } else {
        currentEnergi =
          user?.energi ??
          (m.isOwner || m.isPartner
        ...
        if (currentEnergi < plugin.config.energi) {
          await m.reply(
            config.messages?.energiExceeded || "⚡ Energi habis!",
          );
          return;
        }
...
// =============================================================================
// SECTION 8: GROUP PARTICIPANTS HANDLER
// =============================================================================

/**
 * Handler untuk update group participants (add, remove, promote, demote).
 * @param {Object} update - Update data dari Baileys.
 * @param {Object} sock - Socket connection.
 * @returns {Promise<void>}
 */
        ...
        if (!groupHandler._promoteImg) {
          try {
            groupHandler._promoteImg = fs.readFileSync(
              "./assets/images/ShoNhe-promote.jpg",
            );
          } catch {
        ...
        if (!groupHandler._demoteImg) {
          try {
            groupHandler._demoteImg = fs.readFileSync(
              "./assets/images/ShoNhe-demote.jpg",
            );
          } catch {
...
// =============================================================================
// SECTION 9: MESSAGE UPDATE HANDLER (Anti-Delete / Edit)
// =============================================================================

/**
 * Handler untuk pesan yang di-update atau di-edit.
 * @param {Array<Object>} updates - Array update dari Baileys.
 * @param {Object} sock - Socket connection.
 * @returns {Promise<void>}
 */
...
// =============================================================================
// SECTION 10: GROUP SETTINGS HANDLER (Announce / Restrict)
// =============================================================================
...
/**
 * Handler untuk perubahan setting grup (announce, restrict).
 * @param {Object} update - Update data dari Baileys.
 * @param {Object} sock - Socket connection.
 * @returns {Promise<void>}
 */
